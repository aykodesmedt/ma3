<section class="headerbeeld">
  <div>
    <p class="header--p-datum">24-26 AUG</p>
    <p class="header--p-editie">13e editie</p>
    <p class="header--p-gratis">GRATIS</p>
  </div>
  <div class="headerbeeld--images">
    <h1 class="hidden">Internationaal Straattheaterfestival Beveren</h1>
    <img class="headerbeeld--image1 image--klein" src="./assets/img/logo_groot_320.png" alt="groot logo van ISB" width="274" height="115">
    <img class="headerbeeld--image1 image--groot" src="./assets/img/logo_groot_1440.png" alt="grootste logo van ISB" width="717" height="299">
    <img class="headerbeeld--image2 image--groot" src="./assets/img/headerbeeld.png" alt="grote foto van een meisje op een festival">
    <img class="headerbeeld--image2 image--klein" src="./assets/img/headerbeeld_320.png" alt="kleine foto van een meisje op een festival">
  </div>
</section>
<section class="section--familiedag">
  <h2 class="page--title-underlined home_titel">Zaterdag Familiedag</h2>
  <div>
    <article class="article--voorstelling">
      <div class="container--voorstelling-plaats">
          <div class="container--div-voorstelling">
            <a href="index.php?page=detail&amp;id=21&amp;dag=2">
              <div class="container--div-voorstelling-foto">
                <span class="page--tag page--tag-home tag-1">Acrobatie</span>
                <span class="page--tag page--tag-home tag-2">Voorstelling</span>
                <img src="./assets/img/acts/jpg/024-cie-krak-viva-victor_home.jpg" alt="een man die een lampje vervangt">
              </div>
            </a>
            <div class="container--div-voorstelling-info">
              <div class="container--voorstelling-info">
                <h3 class="title--voorstelling-act">Viva Victor</h3>
                <p class="title--voorstelling-group">by Cie Krak</p>
                <p class="p--voorstelling-uren">13.45 - 15.45</p>
                <a href="index.php?page=detail&id=21&dag=2" class="link--meerWeten"><p>MEER WETEN</p><img src="./assets/img/pijltje.png" alt="pijlte naar rechts"></a>
              </div>
            </div>
          </div>

        <a href="index.php?page=programma&action=filter&dag=2&act=voorstelling" class="button--allevoorstellingen"><p>Alle voorstellingen op zaterdag</p></a>
      </div>
    </article>
    <article class="article--straatact">
      <div class="container--voorstelling-plaats">
        <div class="container--div-straatact">
          <a href="index.php?page=detail&amp;id=27&amp;dag=2">
            <div class="container--div-straatact-img">
              <img src="./assets/img/kijker2_320.png" alt="twee groene wezens die rondlopen met bloemen op hun hoofd">
            </div>
          </a>
          <div class="container--div-straatact-info">
            <div>
              <span class="page--tag">Acrobatie</span>
              <span class="page--tag">Straatact</span>
            </div>
            <div class="container--voorstelling-info">
              <h3 class="title--voorstelling-act" lang="de">Vogelbaum und Blütenfee</h3>
              <p class="title--voorstelling-group">by Pantao</p>
              <p class="p--voorstelling-uren">13.45 - 15.45</p>
              <a href="index.php?page=detail&id=27&dag=2" class="link--meerWeten"><p>MEER WETEN</p><img src="./assets/img/pijltje.png" alt="pijlte naar rechts"></a>
            </div>
          </div>
        </div>
        <a href="index.php?page=programma&action=filter&dag=2&act=straatact" class="button--allevoorstellingen"><p>Alle straatacts op zaterdag</p></a>
      </div>
    </article>
  </div>
</section>
<section class="section--isb-over">
  <h3 class="title--isb-letters"><span>I</span><span>S</span><span>B</span></h3>
  <p class="p--isb-uitleg">
    Internationaal Straattheaterfestival Beveren is een evenement
    opgericht door <span class="link"><a href="http://beverse-feesten.be/">Beveren Verbindt</a></span>. Dit jaar vind de <span class="page--highlight">13e editie</span> van dit
    evenement plaats. Dankzij het succes van de vorige editie blijft de
    locatie van het evenement hetzelfde dit jaar. <br/> <br/>ISB zorgt voor <span class="page--highlight">humor,
    (lucht)acrobatie, vuuracts en stuntwerk</span> van de bovenste plank. Deze
    acts worden uitgevoerd door groepen die uit wel <span class="page--highlight">10 verschillende
    landen </span> komen: België, Nederland, Duitsland, Frankrijk, Spanje, Italië,
    Groot-Brittanië, Argentinië, Guatemala en Canada.
  </p>
</section>
<section class="section--vrijwilligers">
  <h3 class="title--vrijwilligers">Vrijwilliger</h3>
  <p class="p--vrijwilligers">
    Om ons festival zo aangenaam mogelijk te maken voor artiesten en
    publiek doen wij beroep op <span class="page--highlight">vrijwilligers</span>. <br/><br/>
    Wil je het festival in 2018 ook eens langs de andere kant meemaken en de zalige sfeer <span class="page--highlight">achter de
    schermen</span> opsnuiven? <br/><br/>
    Kijk dan zeker eens wat de <span class="page--highlight">voorwaarden</span> zijn en wat je van ons en het werken mag verwachten.
  </p>
  <a href=""><span class="button--allevoorstellingen voorwaarden">Voorwaarden<span></a>
</section>
<section class="section--andereevenementen">
  <h2 class="page--title-underlined underlined-home">Andere evenementen in Beveren</h2>
  <div class="container--evenementen">
    <article class="article--evenementen event1">
      <div class="container--evenement-foto">
        <span class="page--tag tag--datum">24-25-26/8</span>
        <picture>
          <source srcset="./assets/img/andere_evenementen/webp/gekkefietsencircuit.webp 300w,
            ./assets/img/andere_evenementen/webp/gekkefietsencircuit@0,85x.webp 255w,
            ./assets/img/andere_evenementen/webp/gekkefietsencircuit@0,5x.webp 150w,
            ./assets/img/andere_evenementen/webp/gekkefietsencircuit@0,33x.webp 99w"
            sizes="
            (min-width: 1024px) 50vw,
            (min-width: 768px) 10vw,
            (min-width: 320px) 5vw"
            type="image/webp">
          <source srcset="./assets/img/andere_evenementen/jpg/gekkefietsencircuit.jpg 300w,
          ./assets/img/andere_evenementen/jpg/gekkefietsencircuit@0,85x.jpg 255w,
          ./assets/img/andere_evenementen/jpg/gekkefietsencircuit@0,5x.jpg 150w,
          ./assets/img/andere_evenementen/jpg/gekkefietsencircuit@0,33x.jpg 99w"
            sizes="
            (min-width: 1024px) 50vw,
            (min-width: 768px) 10vw,
            (min-width: 320px) 5vw"
            type="image/jpeg">
          <img src="./assets/img/andere_evenementen/jpg/gekkefietsencircuit.jpg" alt="meisjes van de chiro die op fietsen en pluchen dieren zitten" width="100%">
        </picture>
      </div>
      <h3 class="title--evenement">Gekkefietsencircuit</h3>
      <div class="container--evenement-beschrijving">
        <p>
          Scouts Sint-Martinus “on wheels” zijn de gidsen op het knotsgekke
          gekkefietsenparcours. Lol trappen krijgt plots een andere betekenis.
          Dit jaar opnieuw uitgebreid met “diertjes op wielen”.
        </p>
        <a href="http://beverse-feesten.be/randactiviteiten-2018/" class="link--meerWeten"><p>MEER WETEN</p><img src="./assets/img/pijltje.png" alt="pijlte naar rechts"></a>
      </div>
    </article>
    <article class="article--evenementen event2">
      <div class="container--evenement-foto">
        <span class="page--tag tag--datum">26/8</span>
        <picture>
        <source srcset="./assets/img/andere_evenementen/webp/olympischestraat.webp 300w,
            ./assets/img/andere_evenementen/webp/olympischestraat@0,85x.webp 255w,
            ./assets/img/andere_evenementen/webp/olympischestraat@0,5x.webp 150w,
            ./assets/img/andere_evenementen/webp/olympischestraat@0,33x.webp 99w"
            sizes="
            (min-width: 1024px) 50vw,
            (min-width: 768px) 10vw,
            (min-width: 320px) 5vw"
            type="image/webp">
          <source srcset="./assets/img/andere_evenementen/jpg/olympischestraat.jpg 300w,
          ./assets/img/andere_evenementen/jpg/olympischestraat@0,85x.jpg 255w,
          ./assets/img/andere_evenementen/jpg/olympischestraat@0,5x.jpg 150w,
          ./assets/img/andere_evenementen/jpg/olympischestraat@0,33x.jpg 99w"
            sizes="
            (min-width: 1024px) 50vw,
            (min-width: 768px) 10vw,
            (min-width: 320px) 5vw"
            type="image/jpeg">
          <img src="./assets/img/andere_evenementen/jpg/olympischestraat.jpg" alt="kinderen die op straat hockey aan het spelen zijn" width="100%">
        </picture>
      </div>
      <h3 class="title--evenement">13e Olympische Straat</h3>
      <div class="container--evenement-beschrijving">
        <p>
          Breng tijdens de slotdag van de Beverse Feesten een bezoekje aan de
          sportiefste straat van Beveren. Sportclubs komen de straat op en
          laten je genieten van leuke demo’s en doe-mee activiteiten.
        </p>
        <a href="http://beverse-feesten.be/randactiviteiten-2018/" class="link--meerWeten"><p>MEER WETEN</p><img src="./assets/img/pijltje.png" alt="pijlte naar rechts"></a>
      </div>
    </article>
    <article class="article--evenementen event3">
      <div class="container--evenementen-foto">
        <span class="page--tag tag--datum">24/8</span>
        <picture>
        <source srcset="./assets/img/andere_evenementen/webp/avondfeestmarkt.webp 300w,
            ./assets/img/andere_evenementen/webp/avondfeestmarkt@0,85x.webp 255w,
            ./assets/img/andere_evenementen/webp/avondfeestmarkt@0,5x.webp 150w,
            ./assets/img/andere_evenementen/webp/avondfeestmarkt@0,33x.webp 99w"
            sizes="
            (min-width: 1024px) 50vw,
            (min-width: 768px) 10vw,
            (min-width: 320px) 5vw"
            type="image/webp">
          <source srcset="./assets/img/andere_evenementen/jpg/avondfeestmarkt.jpg 300w,
          ./assets/img/andere_evenementen/jpg/avondfeestmarkt@0,85x.jpg 255w,
          ./assets/img/andere_evenementen/jpg/avondfeestmarkt@0,5x.jpg 150w,
          ./assets/img/andere_evenementen/jpg/avondfeestmarkt@0,33x.jpg 99w"
            sizes="
            (min-width: 1024px) 50vw,
            (min-width: 768px) 10vw,
            (min-width: 320px) 5vw"
            type="image/jpeg">
          <img src="./assets/img/andere_evenementen/jpg/avondfeestmarkt.jpg" alt="tentjes naast elkaar met heel wat mensen die rondkijken" width="100%">
        </picture>
      </div>
      <h3 class="title--evenement">Verlichte Avondfeestmarkt</h3>
      <div class="container--evenement-beschrijving">
        <p>
          Het centrum van Beveren wordt opnieuw omgetoverd tot één grote
          feestmarkt. Op en rond de Grote Markt en in de voornaamste
          winkelstraten staan tal van marktkramers opgesteld. <br/>Ook de
          handelaars uit het centrum van Beveren..
        </p>
        <a href="http://beverse-feesten.be/randactiviteiten-2018/" class="link--meerWeten"><p>MEER WETEN</p><img src="./assets/img/pijltje.png" alt="pijlte naar rechts"></a>
      </div>
    </article>
    <article class="article--evenementen event4">
      <div class="container--evenement-foto">
        <span class="page--tag tag--datum">24-25/6</span>
        <picture>
          <source
            srcset="./assets/img/andere_evenementen/webp/rommelmarkt.webp 300w,
            ./assets/img/andere_evenementen/webp/rommelmarkt@0,85x.webp 255w,
            ./assets/img/andere_evenementen/webp/rommelmarkt@0,5x.webp 150w,
            ./assets/img/andere_evenementen/webp/rommelmarkt@0,33x.webp 99w"
            sizes="
            (min-width: 1024px) 50vw,
            (min-width: 768px) 10vw,
            (min-width: 320px) 5vw"
            type="image/webp">
          <source
            srcset="./assets/img/andere_evenementen/jpg/rommelmarkt.jpg 300w,
            ./assets/img/andere_evenementen/jpg/rommelmarkt@0,85x.jpg 255w,
            ./assets/img/andere_evenementen/jpg/rommelmarkt@0,5x.jpg 150w,
            ./assets/img/andere_evenementen/jpg/rommelmarkt@0,33x.jpg 99w"
            sizes="
            (min-width: 1024px) 50vw,
            (min-width: 768px) 10vw,
            (min-width: 320px) 5vw"
            type="image/jpeg">
          <img src="./assets/img/andere_evenementen/jpg/rommelmarkt.jpg" alt="een parkje waar mensen naar spullen uit de kraampjes kijken" width="100%">
        </picture>
      </div>
      <h3 class="title--evenement">Rommelmarkt</h3>
      <div class="container--evenement-beschrijving">
        <p>
          Kuieren langs kraampjes en standjes, unieke koopjes op de kop
          tikken... of zelf uw voorraad aan de man brengen, het blijft een
          traditie op de Beverse feesten die we wat graag in ere houden.
        </p>
        <a href="http://beverse-feesten.be/randactiviteiten-2018/" class="link--meerWeten"><p>MEER WETEN</p><img src="./assets/img/pijltje.png" alt="pijlte naar rechts" width="20" height="8"></a>
      </div>
    </article>
  </div>

</section>
